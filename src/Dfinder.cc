...
{//{{{
    ...
    private:
        ...
        bool isCentrality_;
        int centrality;
        edm::EDGetTokenT<int> tok_centBinLabel_;//Milan
        edm::EDGetTokenT<reco::Centrality> tok_centSrc_;//Milan        
...
//Milan Centrality
    ...
    EvtInfo.centrality = centrality; //Milan
    ...
    */
    ...
    try{
        ...
        memset(genTrackPtr,0x00,MAX_GEN*sizeof(genTrackPtr[0]));    ///// Milan added based on Jing's code
            ...
            if (input_tracks.size() == 0){
                ...
                if (input_tracks.size() > 0){
                    ...
                    for(std::vector<pat::GenericParticle>::const_iterator tk_it=input_tracks.begin();
                        ...
                        //if (tk_it->pt()==2.54283) cout <<" passing "<<2.54283<<endl ;
                        //if (tk_it->pt()==1.32967) cout <<" passing "<<1.32967<<endl ;
                        //cout << tk_it->pt()<< " "<<(tk_it->track()->eta() )  <<" "<< tk_it->phi()<<endl ;
                        ...
                        if(doTkPreCut_){
                            ...
                            if(tk_it->track()->normalizedChi2()/tk_it->track()->hitPattern().trackerLayersWithMeasurement() > 0.18) continue;
                            //outdated selections
                    ...
                    for(std::vector<pat::GenericParticle>::const_iterator tk_it=input_tracks.begin();
                        ...
                        //TrackInfo.fpbarrelhit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInFirstPixelBarrel();  //Milan added based on Jing's code
                        //TrackInfo.fpendcaphit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInFirstPixelEndcap();  //Milan added based on Jing's code
                        ...
                        TrackInfo.dxyErr         [TrackInfo.size] = sqrt(tk_it->track()->d0Error()*tk_it->track()->d0Error()+thePrimaryV.xError()*thePrimaryV.xError() );
                        //TrackInfo.dszPV          [TrackInfo.size] = tk_it->track()->dsz(RefVtx);
                        TrackInfo.dszPV          [TrackInfo.size] = tk_it->track()->dz(RefVtx);
                        //TrackInfo.dszErr         [TrackInfo.size] = tk_it->track()->dszError();
                        TrackInfo.dszErr         [TrackInfo.size] = sqrt(tk_it->track()->dzError()*tk_it->track()->dzError()+thePrimaryV.zError()*thePrimaryV.zError() );
                ...
                for(int nGenDa = 0; nGenDa < std::min(4,int((*sCands)->numberOfDaughters())); nGenDa++){
                    ...
                    mCands != sel_cands.end(); mCands++){
                        if((*sCands)->daughter(nGenDa) == *mCands){
                            ...
                            //cout <<(*mCands)->pt()<< "check some "<<GenInfo.pt[GenInfo.da1[geninfo_idx]]<<endl;
                            
                        }
                    }
                }
                            if (fabs(GenInfo.pdgId[geninfo_idx]) == 421) cout << "check some "<<GenInfo.pt[GenInfo.da1[geninfo_idx]]<<endl;
...
//Milan dcls
        ...
        SMatrixSym3D totalCov = thePrimaryV.covariance() + tktk_VFPvtx->vertexState().error().matrix();
        SVector3 distanceVector(tktk_VFPvtx->position().x()-thePrimaryV.position().x(),tktk_VFPvtx->position().y()-thePrimaryV.position().y(),tktk_VFPvtx->position().z()-thePrimaryV.position().z());

//Milan end dcls
//Milan lVtxSigCut
        GlobalVector d0LineOfFlight = GlobalVector (tktk_VFPvtx->position().x()-thePrimaryV.position().x(),tktk_VFPvtx->position().y()-thePrimaryV.position().y(),tktk_VFPvtx->position().z()-thePrimaryV.position().z());
        std::vector<double> d0VtxEVec;
        d0VtxEVec.push_back( tktk_VFPvtx->vertexState().error().cxx() );
        d0VtxEVec.push_back( tktk_VFPvtx->vertexState().error().cyx() );
        d0VtxEVec.push_back( tktk_VFPvtx->vertexState().error().cyy() );
        d0VtxEVec.push_back( tktk_VFPvtx->vertexState().error().czx() );
        d0VtxEVec.push_back( tktk_VFPvtx->vertexState().error().czy() );
        d0VtxEVec.push_back( tktk_VFPvtx->vertexState().error().czz() );
        SMatrixSym3D d0VtxCovMatrix(d0VtxEVec.begin(), d0VtxEVec.end());

        SMatrixSym3D d0TotalCov;
        //if(isVtxPV) 
        d0TotalCov = d0VtxCovMatrix + thePrimaryV.covariance();
        //else d0TotalCov = d0VtxCovMatrix + theBeamSpotHandle->rotatedCovariance3D();

//Milan end lVtxSigCut
