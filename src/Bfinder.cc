                ...
                if (input_tracks.size() > 0 && input_muons.size() > 1){
                    ...
                    for(std::vector<pat::Muon>::const_iterator mu_it=input_muons.begin();
                        ...
                        if(mu_it->innerTrack().isNonnull()){
                            ...
                            //MuonInfo.fpbarrelhit             [MuonInfo.size] = mu_it->innerTrack()->hitPattern().hasValidHitInFirstPixelBarrel(); //Milan based on Jing's code
                            //MuonInfo.fpendcaphit             [MuonInfo.size] = mu_it->innerTrack()->hitPattern().hasValidHitInFirstPixelEndcap(); //Milan based on Jing's code
                    ...
                    for(std::vector<pat::GenericParticle>::const_iterator tk_it=input_tracks.begin();
                        ...
                        //TrackInfo.fpbarrelhit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInFirstPixelBarrel();   //Milan based on Jing's code
                        //TrackInfo.fpendcaphit    [TrackInfo.size] = tk_it->track()->hitPattern().hasValidHitInFirstPixelEndcap();   //Milan based on Jing's code
